%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% --------------------------------------------------------------------- %%
%                FORWARD RESPONSE (REFLECTION COEFFICIENT)                %
% ----------------------------------------------------------------------- %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%  [FWD_IP,FWD_QP] = FDEM1DFWD_RC(S,M)
%
%  Use:
...
function [FWD_IP,FWD_QP] = FDEM1DFWD_RC(S,M)
...
function y = digitalFilter(funname,order,S,M,mu0,eps0)
    ...
    %
        ...
        if strcmp(funname,'rTE01')
            YF = rTE01(lambda,S,mu0,eps0);
        elseif strcmp(funname,'rTE02')
            YF = rTE02(lambda,S,mu0,eps0);
        elseif strcmp(funname,'rTEn1')
            YF = rTEn1(lambda,S,M,mu0,eps0);
        elseif strcmp(funname,'rTEn2')
            YF = rTEn2(lambda,S,M,mu0,eps0); 
        elseif strcmp(funname,'rTEp2')
            YF = rTEp2(lambda,S,M,mu0,eps0);
        end
...
%
%  Use:
%  Calculates the reflection coefficient for a given layered half-space and
%  lambda value.
%
%  Input:
%  S (structure)           Sensor characteristics
...
function rTE = reflectionCoefficient(lambda,S,M,mu0,eps0)
    ...
    %
        ...
        u= sqrt(repmat(lambda.^2,1,length(M.con)) - ...
                repmat(S.omega.^2.*mu.*M.perm,numel(lambda),1) + ...
                repmat(1i.*S.omega.*mu.*M.con,numel(lambda),1));
    ...
    %
        ...
        Y0= u0 ./ (1i.*S.omega.*mu0);
        Y= u ./ (repmat(1i.*S.omega.*mu,numel(lambda),1));
        Yhat = Y;
    ...
    %
        ...
        for ii = numel(M.con)-1:-1:1

            %
            % Calculate tanh
            %
            
                tanh_uh = tanh(u(:,ii).*M.thick(ii));
            ...
            %
                ...
                num = Yhat(:,ii+1) + Y(:,ii) .* tanh_uh;
                den = Y(:,ii) + Yhat(:,ii+1) .* tanh_uh;
                Yhat(:,ii) = Y(:,ii) .* num ./ den;
...
% ----------------------------------------------------- Function 1/5 ---- %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function y = rTE01(lambda,S,mu0,eps0)
...
% ----------------------------------------------------- Function 2/5 ---- %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function y = rTE02(lambda,S,mu0,eps0)
...
% ----------------------------------------------------- Function 3/5 ---- %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function y = rTEn1(lambda,S,M,mu0,eps0)
       
    %
    % Calculate rTE
    %
    
        rTE= reflectionCoefficient(lambda,S,M,mu0,eps0);
    ...
    %
        ...
        y= (-rTE.*exp(u0.*(S.z-S.height))).*lambda;
...
% ----------------------------------------------------- Function 4/5 ---- %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function y = rTEn2(lambda,S,M,mu0,eps0)
    ...
    %
        ...
        y= (-rTE.*exp(u0.*(S.z-S.height))).*lambda.^2;
...
% ----------------------------------------------------- Function 5/5 ---- %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function y = rTEp2(lambda,S,M,mu0,eps0)
    ...
    %
        ...
        y= (rTE.*exp(u0.*(S.z-S.height))).*lambda.^3./u0;
